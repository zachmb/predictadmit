...
<div class="min-h-screen font-sans text-slate-800 bg-white">
	{#if !authenticated}
		...
		<main class="bg-white min-h-[500px] pt-8 pb-12">
			<div class="max-w-xl mx-auto px-6 text-[13px]">
				<h1 class="text-2xl font-normal mb-6 text-slate-900">Login</h1>

				<div
					class="bg-green-50 px-4 py-3 mb-6 text-[13px] text-slate-900 font-normal border border-green-200 max-w-full"
				>
	...
	{:else if !hasViewedUpdate}
			...
			<div class="max-w-6xl mx-auto px-6">
				<div class="max-w-4xl mx-auto text-sm">
					<div class="flex justify-end text-[12px] font-semibold text-slate-600">
						{applicantName()}
						<a href="/disclaimer" class="text-blue-700 hover:underline ml-1">Logout</a>
					</div>
					...
					<div
						class="mb-8 p-4 border-l-4"
						style="border-color: {school.primaryColor}; background-color: {school.secondaryColor};"
					>
						<p class="text-base text-black">
							Admission decisions for Regular Decision (and deferred Early Decision) candidates will
							be available **{school.decisionDateText}**.
						</p>
					</div>

					<div class="flex gap-1 border-b border-gray-300 mb-8">
						<button
							class="border-b-2 font-semibold px-4 py-2 text-sm"
							style="border-color: {school.primaryColor}; color: {school.primaryColor};"
						>
							Admissions
						</button>
						<button
							class="text-gray-600 border-b-2 border-transparent px-4 py-2 text-sm hover:border-gray-400"
						>
							Financial Aid
						</button>
					</div>
					...
					<p class="text-sm text-slate-600">All application documents have been received.</p>
		...
		<footer
			style="background-color: {school.primaryColor};"
			class="text-white py-6 text-sm font-normal"
		>
			<div class="max-w-6xl mx-auto px-6">
				<div
					class="flex justify-between items-start text-[12px] pb-4 border-b border-gray-500 mb-4"
				>
