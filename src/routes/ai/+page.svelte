<script lang="ts">
	...
	let ocrError = $state('');
	let ocrNeedsSignIn = $state(false);
	let ocrText = $state('');
	...
	async function handleOcrChange(event: Event) {
		...
		// Reset so picking the SAME file again re-fires onchange (a failed attempt
		// otherwise looks like the uploader silently ignoring the retry).
		target.value = '';

		ocrError = '';
		ocrNeedsSignIn = false;
		ocrText = '';

		if (!file) return;

		// The API requires a session; say so up front instead of letting the
		// upload bounce off a 401 that reads like a broken parser.
		if (!googleSignedIn) {
			ocrNeedsSignIn = true;
			ocrError = 'Sign in with Google first. Your upload only takes a second after that.';
			return;
		}

		// Guard: only PDFs (some browsers leave file.type empty — fall back to the name).
		const looksLikePdf =
			file.type === 'application/pdf' ||
			(file.type === '' && file.name.toLowerCase().endsWith('.pdf'));
		if (!looksLikePdf) {
			ocrError = 'That’s not a PDF. Export your resume as a PDF, or paste the text below.';
			return;
		}

		// Vercel drops request bodies over ~4.5 MB before the server ever sees
		// them, which surfaces as a dead upload — refuse early with a real reason.
		if (file.size > 4 * 1024 * 1024) {
			ocrError = 'That PDF is over 4 MB. Export a smaller copy, or paste the text below.';
			return;
		}
		...
		try {
			...
			if (!res.ok) {
				...
				try {
					...
					/* non-JSON body (proxy error page) — fall through to defaults */
				}
				console.error('OCR error:', res.status, serverError);
				if (res.status === 401) {
					// Session expired mid-visit (the pre-check above covers fresh loads).
					googleSignedIn = false;
					ocrNeedsSignIn = true;
					ocrError = 'Your session expired. Sign in with Google again to upload.';
				} else if (res.status === 429) {
					ocrError = serverError || 'You’re going a bit fast. Give it a moment and try again.';
				} else if (res.status === 413) {
					ocrError = 'That PDF is over 4 MB. Export a smaller copy, or paste the text below.';
				} else {
					ocrError =
						serverError ||
						'That PDF didn’t read cleanly. Try a different one, or just paste the text.';
				}
				return;
			}

			const data = (await res.json()) as { text?: string; scanned?: boolean; message?: string };
			ocrText = (data.text ?? '').trim();

			if (!ocrText) {
				ocrError =
					data.message ??
					'Read the file but found no text. It’s probably a scanned image or locked.';
			} else {
	...
	async function parseAndFill(text: string) {
		...
		try {
			...
			if (!res.ok) {
				if (res.status === 401) {
					// Session expired mid-visit — flip the buttons back to sign-in mode.
					googleSignedIn = false;
					parseError = 'Your session expired. Sign in with Google again, then hit Autofill.';
				} else {
						...
						>
							...
							<div class="grid gap-6">
								...
								>
									...
									{#if !googleSignedIn}
										<!-- The upload API needs a session — ask for it up front instead of
										     letting the file bounce off a 401 that looks like a broken reader. -->
