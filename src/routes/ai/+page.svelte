<script lang="ts">
	...
	import { aiResults, type AiDecision, type DecisionOutcome } from '$lib/stores/results';
	import { aiEvaluationService } from '$lib/services/aiEvaluationService';
	import { SCHOOLS } from '$lib/config/aiSchools';
	...
	// 🔐 LocalStorage persistence key for AI inbox view state (read/unread, selection)
	...
	// Free-tier limits
	let hasUsedFreeSimulation = false;
	let hasUsedFreePdfOcr = false;
	let promoCodeInput = '';
	...
	// Derived from store
	$: isSubmitting = $aiResults.isEvaluating;
	let aiError = '';
	...
	// Reactive visible portals from decisions
	...
	function outcomeClasses(outcome: DecisionOutcome): string {
		if (outcome === 'admit') return 'bg-emerald-500/15 text-emerald-300 border-emerald-400/70';
		if (outcome === 'deny') return 'bg-rose-500/10 text-rose-300 border-rose-400/70';
		if (outcome === 'waitlist') return 'bg-amber-500/15 text-amber-300 border-amber-400/70';
	...
	async function runEvaluation() {
		...
		try {
			await aiEvaluationService.startEvaluation({
				essay,
				activities,
				honors,
				transcript,
			...
			console.error(err);
			aiError = 'System error starting evaluation.';
		}
	}
...
<main
	class="relative min-h-screen bg-slate-50 text-slate-900 overflow-hidden font-sans animate-enter"
>
	<div class="">
							...
							<div class="flex items-center gap-3">
								{#if googleSignedIn}
									<div class="text-xs text-right">
										<div class="font-bold text-slate-900">{googleName || 'Signed in'}</div>
										<div class="text-slate-500">{googleEmail}</div>
									</div>
								{:else}
									<button
										type="button"
						...
						>
								...
								<div class="rounded-lg bg-slate-50 p-4 border border-slate-100">
									...
									<label class="flex items-center gap-3 cursor-pointer group">
										...
										<span class="text-xs text-slate-400">
											{#if ocrUploading}Extracting...{:else if ocrText}Text extracted ready for
												essay.{:else}Common App PDF only.{/if}
										</span>
									</label>
									{#if ocrError}<p class="mt-2 text-xs text-rose-400">{ocrError}</p>{/if}
									{#if ocrText}
										<div class="mt-3">
											<button
												type="button"
							...
							<div class="flex flex-col sm:flex-row items-center gap-4 pt-4">
								...
								>
									...
									>
										...
										>
											...
											{:else}
												<span>Get Application Results (PredictAdmit AI)</span>
											{/if}
										</div>
									</div>
								</button>
								<button
									...
									on:click={resetInboxState}>Clear AI inbox</button
								>
							</div>
							...
							{#if applicantSummary}
								<details class="mt-4 text-xs text-slate-500">
									<summary class="cursor-pointer font-semibold text-slate-700 hover:text-slate-900"
										>Preview extracted text</summary
									>
									<pre
										class="mt-2 max-h-48 overflow-auto whitespace-pre-wrap text-[10px] text-slate-600 border border-slate-200 rounded-md p-3 bg-slate-50">{applicantSummary}</pre>
								</details>
							{/if}
						</form>
					</div>
				...
				>
					...
					>
						...
						<div class="text-right text-[10px] text-slate-400">
							<span class="font-medium text-slate-600">{googleEmail || 'you@predictadmit.ai'}</span>
						</div>
					</div>
								...
								{#each deepDiveItems as item (item.slug)}
									<article class="rounded-lg border border-slate-200 bg-white px-5 py-4 shadow-sm">
										<div class="flex flex-wrap items-center justify-between gap-2 mb-3">
											<div class="flex items-center gap-3">
												<h3 class="text-sm font-bold text-slate-900">{item.school}</h3>
												<span
													class={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide border ${item.outcome === 'admit' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : item.outcome === 'deny' ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}
												>
		...
		>
			...
			>
				<div class="flex items-center justify-between gap-3">
					<div class="space-y-1">
						<p class="text-[10px] uppercase tracking-widest font-bold text-emerald-600">
							{#if paywallMode === 'deepDive'}Locked · Deep Dive Explanation{:else if paywallMode === 'simulation'}Locked
								· Extra AI Simulations{:else}Locked · Extra PDF Uploads{/if}
						</p>
						<h2 class="text-lg font-bold text-slate-900">
							{#if paywallMode === 'deepDive' && paywallContextDecision}
								You know the verdict. Now learn <span class="text-emerald-600">why</span>.
				...
				<div class="space-y-4 text-sm text-slate-600 leading-relaxed text-left">
					{#if paywallMode === 'deepDive' && paywallContextDecision}
						<p>
							Right now you’re staring at a single line — <span class="font-bold text-slate-900"
						...
						<p>
							...
							unlock PredictAdmit <span class="font-bold text-emerald-600">Pro</span>.
						</p>
						<div class="mt-4">
							<button
								on:click={() => goto('/pro')}
					...
					{:else}
						...
						<p>
							...
							to unlock PredictAdmit <span class="font-bold text-emerald-600">Pro</span>.
						</p>
						<div class="mt-4">
							<button
								on:click={() => goto('/pro')}
				...
				<div class="space-y-3 pt-2">
					...
					>
						{#if paywallMode === 'deepDive'}Unlock Deep Dive explanations{:else if paywallMode === 'simulation'}Unlock
							more HYPSM+ simulations{:else}Unlock more Common App uploads{/if}
					</a>
					<button
						type="button"
						...
						on:click={closePaywall}>Not now · keep the free run</button
					>
