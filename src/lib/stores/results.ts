...
function createResultsStore() {
	...
	const initialValue: AiResultsPayload = stored ? JSON.parse(stored) : { decisions: [], raw: null };
...
export const academicScoresBySlug = derived(aiResults, ($results) => {
	...
	for (const d of $results.decisions) {
		if (d.slug) map[d.slug] = d.academic_score;
	}
	return map;
});
...
export const academicExplanationsBySlug = derived(aiResults, ($results) => {
	...
	for (const d of $results.decisions) {
		if (d.slug) map[d.slug] = d.academic_explanation;
	}
	return map;
});
...
export const extracurricularScoresBySlug = derived(aiResults, ($results) => {
	const map: Record<string, number> = {};
	for (const d of $results.decisions) {
		if (d.slug) map[d.slug] = d.extracurricular_score;
	}
	return map;
});
...
export const extracurricularExplanationsBySlug = derived(aiResults, ($results) => {
	const map: Record<string, string> = {};
	for (const d of $results.decisions) {
		if (d.slug) map[d.slug] = d.extracurricular_explanation;
	}
	return map;
});
...
export const fitScoresBySlug = derived(aiResults, ($results) => {
	const map: Record<string, number> = {};
	for (const d of $results.decisions) {
		if (d.slug) map[d.slug] = d.fit_score;
	}
	return map;
});
...
export const fitExplanationsBySlug = derived(aiResults, ($results) => {
	const map: Record<string, string> = {};
	for (const d of $results.decisions) {
		if (d.slug) map[d.slug] = d.fit_explanation;
	}
	return map;
});
...
export const intellectualScoresBySlug = derived(aiResults, ($results) => {
	const map: Record<string, number> = {};
	for (const d of $results.decisions) {
		if (d.slug) map[d.slug] = d.intellectual_score;
	}
	return map;
});
...
export const intellectualExplanationsBySlug = derived(aiResults, ($results) => {
	const map: Record<string, string> = {};
	for (const d of $results.decisions) {
		if (d.slug) map[d.slug] = d.intellectual_explanation;
	}
	return map;
});
...
export const characterScoresBySlug = derived(aiResults, ($results) => {
	const map: Record<string, number> = {};
	for (const d of $results.decisions) {
		if (d.slug) map[d.slug] = d.character_score;
	}
	return map;
});
...
export const characterExplanationsBySlug = derived(aiResults, ($results) => {
	const map: Record<string, string> = {};
	for (const d of $results.decisions) {
		if (d.slug) map[d.slug] = d.character_explanation;
	}
	return map;
});
...
export const improvementTipsBySlug = derived(aiResults, ($results) => {
	const map: Record<string, string> = {};
	for (const d of $results.decisions) {
		if (d.slug) map[d.slug] = d.improvement_tips;
	}
	return map;
});
...
export function resolveOutcomeForSlug(
	slug: string,
	map: Record<string, DecisionOutcome>,
	fallback: DecisionOutcome
): DecisionOutcome {
	return map[slug] ?? fallback;
}

export function resolveAcademicScoreForSlug(
	slug: string,
...
export function resolveAcademicExplanationForSlug(
	slug: string,
...
export function resolveExtracurricularScoreForSlug(
	slug: string,
	map: Record<string, number>,
	fallback: number = 0
): number {
...
export function resolveExtracurricularExplanationForSlug(
	slug: string,
	map: Record<string, string>,
	fallback: string = ''
): string {
...
export function resolveFitScoreForSlug(
	slug: string,
	map: Record<string, number>,
	fallback: number = 0
): number {
...
export function resolveFitExplanationForSlug(
	slug: string,
	map: Record<string, string>,
	fallback: string = ''
): string {
...
export function resolveIntellectualScoreForSlug(
	slug: string,
	map: Record<string, number>,
	fallback: number = 0
): number {
...
export function resolveIntellectualExplanationForSlug(
	slug: string,
	map: Record<string, string>,
	fallback: string = ''
): string {
...
export function resolveCharacterScoreForSlug(
	slug: string,
	map: Record<string, number>,
	fallback: number = 0
): number {
...
export function resolveCharacterExplanationForSlug(
	slug: string,
	map: Record<string, string>,
	fallback: string = ''
): string {
...
export function resolveImprovementTipsForSlug(
	slug: string,
	map: Record<string, string>,
	fallback: string = ''
): string {
