<script lang="ts">
	...
	// Optional handoff to the paid Human Counselors tab.
	let { onBookHuman }: { onBookHuman?: () => void } = $props();
						...
						<div class="mt-6 grid w-full grid-cols-1 gap-3 sm:grid-cols-2">
							...
							{#if onBookHuman}
								<button
									type="button"
									onclick={onBookHuman}
									class="mt-6 inline-flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-sm font-semibold text-amber-800 transition hover:bg-amber-100"
								>
									...
									Want a human to review your essay? Book a Top-20 mentor →
								</button>
							{/if}
					</div>
				{:else}
