	...
	<section class="bg-white border border-slate-400 shadow-md rounded-sm">
		...
		<div class="px-4 py-4 grid grid-cols-1 md:grid-cols-3 gap-4 text-[11px]">
			...
			<div class="md:col-span-2 border border-slate-400 rounded-sm shadow-sm bg-slate-50">
				<div
					class="flex items-center justify-between px-3 py-2 border-b border-slate-300 bg-slate-200"
				>
					...
					<div class="text-[10px] text-slate-700">🔒 Secure</div>
				</div>
					...
					{#if applicationPhase === 'commonapp'}
						...
						<div class="border border-slate-200 rounded-sm overflow-hidden text-[11px]">
							...
							<div class="flex border-b border-slate-200 bg-slate-50 text-[10px]">
								...
								<div class="px-4 py-2 text-slate-500">Dashboard</div>
								<div class="px-4 py-2 text-slate-500">My Colleges</div>
								<div class="px-4 py-2 text-slate-500">College Search</div>
							</div>
							<div class="px-4 py-4 grid grid-cols-1 md:grid-cols-2 gap-4">
								<div class="space-y-2">
									<div>
										<label class="block text-[10px] font-semibold mb-1" for="common-first-name">
											First / Given Name
										</label>
										<input
									...
									<div>
										<label class="block text-[10px] font-semibold mb-1" for="common-last-name">
											Last / Family Name
										</label>
										<input
											id="common-last-name"
									...
									<div>
										<label class="block text-[10px] font-semibold mb-1" for="common-email">
											Email Address
										</label>
										<input
											id="common-email"
									...
									<div class="grid grid-cols-2 gap-2">
										<div>
											<label class="block text-[10px] font-semibold mb-1" for="common-password">
												Password
											</label>
											<input
												id="common-password"
								...
								<div class="space-y-3">
									<div>
										<div class="text-[10px] font-semibold mb-1">Profile completion</div>
					...
					{:else if applicationPhase === 'fee'}
						...
						<div class="border border-emerald-200 rounded-sm bg-emerald-50 px-4 py-4 text-[11px]">
							...
							<div class="border border-emerald-200 bg-white rounded-sm px-3 py-3">
								...
								<p class="mb-2 text-slate-700">Applications submitted to:</p>
					...
					{:else}
						<div>
							...
							<p class="text-[10px] text-slate-600">
								Once the calendar reaches December 15, your Early Decision / REA result (if
								applicable) will appear. Regular Decision emails unlock on the March decision date.
							</p>
						</div>
					{/if}
				</div>
			...
			<div
				class="border border-slate-400 rounded-sm bg-slate-50 px-3 py-3 flex flex-col justify-between"
			>
				<div>
					<h3 class="text-xs font-bold text-slate-900 mb-2 uppercase">Time passing</h3>
					<p class="text-[11px] text-slate-800 mb-2">
						The calendar jumps from August application season through December early rounds and then
						to March Regular Decision.
					</p>
				</div>
				<div class="mt-2 border border-slate-300 bg-white px-3 py-2 text-center">
					<div class="text-[10px] uppercase text-slate-500 tracking-wide">Today</div>
					<div class="text-sm font-semibold text-slate-900">
						{calendarDates[calendarIndex]}
					</div>
				</div>
				<p class="mt-3 text-[10px] text-slate-600">
					When the application process has finished and the date reaches your decision rounds,
					admitMail will start to fill with emails.
				</p>
			</div>
		</div>
	</section>
