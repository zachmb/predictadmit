...
<section id="inboxSection" bind:this={inboxSection} class="scroll-mt-24">
	<Card>
		<div
		...
		>
			<div>
				<div class="flex items-baseline gap-2">
					<span class="text-xl font-bold text-[var(--color-brand-primary)] tracking-tight">
						admitMail
					</span>
					<span
						class="inline-flex items-center justify-center text-[10px] font-bold uppercase tracking-wider text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full border border-slate-200"
					>
						Beta
					</span>
				</div>
			...
			<div class="hidden md:flex flex-col items-end text-xs text-slate-600 gap-2">
				...
				<button
					type="button"
					class="text-xs hover:text-red-500 hover:underline transition-colors"
		...
		{#if viewMode === 'inbox'}
			<!-- Search bar -->
			<div
				class="px-4 py-2 border-t border-slate-200 bg-white flex items-center justify-between gap-3"
			>
			...
			<div class="flex text-[11px]">
					...
					<nav class="text-[11px] text-slate-800 space-y-1">
						<button
							...
							class={`w-full text-left px-3 py-1 rounded-r-full ${
								activeFolder === 'inbox' ? 'bg-blue-100 font-semibold' : 'hover:bg-slate-100'
							}`}
						...
						<button
							type="button"
							class={`w-full text-left px-3 py-1 rounded-r-full ${
								activeFolder === 'sent' ? 'bg-blue-100 font-semibold' : 'hover:bg-slate-100'
							}`}
				...
				<div class="flex-1">
					...
					{#if currentEdPortal && edEmailMustBeViewed && !hasViewedEdEmail}
						<div
							class="px-4 py-2 bg-red-50 border-b border-red-200 text-[11px] text-red-800 flex items-center justify-between"
						>
					...
					<div class="border-b border-slate-200 bg-white flex text-[11px]">
						...
						<button type="button" class="px-4 py-2 text-slate-500 hover:bg-slate-50">
							Updates
						</button>
						<button type="button" class="px-4 py-2 text-slate-500 hover:bg-slate-50">
							Promotions
										...
										{:else}
											{#each filteredPortals as portal (portal.slug)}
												...
												>
													...
													<td
														class="px-2 py-2 align-top text-slate-800 text-[11px] whitespace-nowrap w-64"
													>
		...
		{:else}
			...
			<div class="flex text-[11px]">
				...
				<div class="flex-1">
					<!-- Top controls -->
					<div
						class="border-b border-slate-300 bg-slate-100 px-3 py-2 flex items-center justify-between"
					>
							...
							<div class="max-w-3xl text-[12px] leading-relaxed text-slate-900">
								...
								<div class="text-[11px] text-slate-700 mb-3 border-b border-slate-300 pb-2">
									...
									<div>
										...
										{getReceivedLabel(selectedPortal)}
									</div>
								</div>
								...
								<p class="mb-2">
									...
									status for <span class="font-semibold"> {selectedPortal.name}</span>. For security
									reasons, we do not release admission decisions via email.
								</p>
								<p class="mb-2">
								...
								<p class="mb-3">To view your status, click the link below:</p>
