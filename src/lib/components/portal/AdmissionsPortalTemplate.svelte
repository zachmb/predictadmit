		...
		<div class="max-w-5xl mx-auto px-4 py-8">
			...
			<div class="flex justify-end text-sm">
				<div class="text-right">
					{#if admissionsId}
						<div>
							<span class="font-bold">Admissions ID:</span>
							{admissionsId}
						</div>
					{/if}
					{#if financialAidId}
						<div>
							<span class="font-bold">Financial Aid ID:</span>
							{financialAidId}
						</div>
					{/if}
				</div>
			...
			{#if showTabs}
				<div class="mb-6 flex gap-3">
					<button type="button" class="border border-gray-400 bg-gray-200 text-xs px-4 py-1">
						{admissionsTabLabel}
					</button>
					<button
						type="button"
